...
"""
def display_surface(fun,w,trayectoria, labels=['u','v','E(u,v)'], title='', x_min=-10, x_max=10, y_min=-10, y_max=10):
	...
	for i in range(len(trayectoria_20)):		
		ax.plot(trayectoria_20[i][0], trayectoria_20[i][1], E(trayectoria_20[i][0], trayectoria_20[i][1]),
																			   'k+', markeredgewidth=2, 
...
"""
def gradient_descent(w, lr, fun, gradf, iterations=50, error=-np.inf ): # en el video dice que parámetros podemos incluir aquí
...
def ejercicio12():
	...
	display_surface(E,w,trayectoria, title=titulo_grafica, x_min=-7, x_max=1, y_min=-1, y_max=6)
	wait()
...
def ejercicio13a(eta=0.1, initial_point=np.array([-1.0,1.0], np.float64), iteraciones = 50):
	...
	titulo_grafica = 'Ejercicio 1.3.a. Función sobre la que se calcula el descenso de gradiente'	
	
	if(eta==0.1):
		display_surface(f,w,trayectoria,labels=['x','y','f(x,y)'],title=titulo_grafica,x_min=-5, x_max=0, y_min=0, y_max=4)
	else:
		display_surface(f,w,trayectoria,labels=['x','y','f(x,y)'],title=titulo_grafica,x_min=-1.4, x_max=-0.9, y_min=0.9, y_max=1.4)
...
def ejercicio13b():
	...
	for w0 in puntos_inicio:
		...
		print('\tInicio en ('+str(w0[0])+','+str(w0[1])+')')
...
def ejercicio13():
	"""
		Aquí decir reflexiones sobre la función (hablar de los criterios de parada)
	"""
	# Primer apartado
...
def gradErr(X,Y,w):
	...
	for j in range(X.shape[1]):
		...
		for n in range(X.shape[0]):
			res = res + X[n][j]*(np.sum(X[n]*w) - Y[n])
...
def ejercicio22(datos):	
	...
	for iteracion in range(ITERACIONES):
		...
		# Mostramos el mapa
		# display_regresion(x_train[:,1:3], y_train, title='Datos uniformes en [-1,1]x[-1,1]', etiquetas=['naranja', 'verde'], labels_axes = ['x', 'y'])
		
		#print('Ejecutando el SGD con los datos uniformes en [-1,1]x[-1,1]')
		weights, trayectoria = sgd(x_train, y_train, np.array([0.5 for i in range(x_train.shape[1])]), 32, 50, 0.1)
		...
		if(iteracion==0):
			...
			display_regresion(x_train[:,1:x_train.shape[1]], y_train, title='Conjunto train', etiquetas=['naranja', 'verde'],
							   labels_axes = ['Intensidad promedio', 'Simetría'], w=weights, reg=funcion)
			print('\t\tEin: '+ str(Ein))	
...
Método de Newton
"""
def newton_method(w, iteraciones, lr=1):
	trayectoria = [w]
	for i in range(iteraciones):
		w_new = w - lr*np.dot( np.linalg.inv(hessf(w[0],w[1])), gradf(w[0],w[1]))
...
"""""""""""""""""""""""""""
def ejercicio3():
	weights, trayectoria = newton_method(np.array([0.0,0.0]), 200)
                 
	display_surface(f,weights,trayectoria,labels=['x','y','f(x,y)'],title='Método de Newton',x_min=-1.5, x_max=0.5, y_min=-1.5, y_max=0.5)
